...
SESSIONS = [
    ...
    ("admin_setup", [
        ...
        # 1001-02 삭제 (inactive 제외 셋업) — 대상 정보 확인 → 1 → 엔터
        "6", "1001", "02", "1", "",
        # 학생 등록 (6.13-10)
    ...
    ("admin_after", [
        "2026-04-01", "1", "admin01", "Admin@1234",
        "5", "1002", "01", "5", "1", "1",         # 정원 1 → 신청 인원(2명) 차단 (6.14-23)
        "5", "1001", "01", "1", "새과목명", "1",   # 기간 후 과목명 차단 (6.14-19)
        "5", "1001", "01", "5", "50", "1",        # 정원 증가 허용 (6.14-21)
        "5", "1001", "01", "5", "70", "1",        # 최소 좌석 초과 (6.14-22)
        "0", "0",
    ]),
    ("cancel", [   # 1학년 컴공 — 수강취소 재입력/정상 (6.10 재캡처)
        "2026-04-01", "1", "202511111", "Pass123",
        "5", "1001 01", "1",                       # 프로그래밍기초 신청
        "5", "2001 01", "1",                       # 영어회화 신청
        "6", "99", "0",                            # 잘못된 번호 → 재입력 → 돌아가기 (6.10-3)
        "6", "abc", "0",                           # 비숫자 → 재입력 → 돌아가기 (6.10-6)
        "6", "1", "1",                             # 번호1 선택 → 확인 → 취소 완료 (6.10-1)
        "0", "0",
    ]),
]
...
CUTS = [
    ...
    ("6.10-3", "cancel", ("===== 수강취소 =====", 1), "다시 선택하세요.", 0, 1),
    ("6.10-6", "cancel", ("===== 수강취소 =====", 2), "다시 선택하세요.", 0, 1),
    ("6.10-1", "cancel", ("===== 수강취소 =====", 3), "수강취소 완료", 0, 1),
]
